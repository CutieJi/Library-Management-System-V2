...
class Admin extends CI_Controller
{
    ...
    public function user_data()
    {
        ...
        // Last Name Field
        $this->form_validation->set_rules('username', 'Student No.', 'required|trim', ['required' => "Student No. can't be empty"]);
        // Last Name Field
    ...
    $data['pending_users'] = $this->db->get_where('user_data', ['status' => 0])->result_array();

    $this->load->view('layout/layout_header', $data);
    $this->load->view('layout/layout_sidebar');
    $this->load->view('layout/layout_topbar', $data);
    $this->load->view('admin/pending_accounts', $data);
    $this->load->view('layout/layout_footer');
    }

    public function approve($id)
    {
    $user = $this->db->get_where('user_data', ['id' => $id, 'status' => 0])->row_array();

    if ($user){
    
        $this->db->where('id', $id);
        $this->db->update('user_data', ['status' => 1]);
